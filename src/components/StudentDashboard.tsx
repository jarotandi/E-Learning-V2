...
export const StudentDashboard: React.FC<{ setView: (v: View) => void, user: UserType | null, logout: () => void, onUpgrade: () => void }> = ({ setView, user, logout, onUpgrade }) => {
  ...
  const premiumUntil = directoryUser?.premiumUntil || user?.premiumUntil || (hasPremiumAccess ? '31 Des 2025' : '-');
  const accessLabel = hasPremiumAccess ? 'Premium Aktif' : isScholarshipReview || isPaidPending ? 'Menunggu Verifikasi' : 'Akses Terbatas';
      ...
      <main className="flex-1 overflow-y-auto pt-20 lg:pt-0 p-4 lg:p-10">
         <header className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
            ...
            <div className="w-full md:w-auto flex items-stretch gap-3">
               <div className={`card-premium p-4 min-w-full md:min-w-[360px] ${hasPremiumAccess ? 'border-emerald-100 bg-emerald-50/40' : isScholarshipReview ? 'border-indigo-100 bg-indigo-50/40' : 'border-amber-100 bg-amber-50/40'}`}>
                  <div className="flex items-start justify-between gap-4 mb-3">
                     <div className="min-w-0">
                        <span className={`text-[10px] font-black uppercase tracking-wider ${hasPremiumAccess ? 'text-emerald-600' : isScholarshipReview ? 'text-indigo-600' : 'text-amber-600'}`}>{accountLabel}</span>
                        <p className="text-sm font-black text-brand-navy truncate">{packageName}</p>
                     </div>
                     <div className={`shrink-0 p-2 rounded-xl ${hasPremiumAccess ? 'bg-emerald-100 text-emerald-600' : isScholarshipReview ? 'bg-indigo-100 text-indigo-600' : 'bg-amber-100 text-amber-600'}`}>
                        <ShieldCheck size={18} />
                     </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3 text-xs">
                     <div>
                        ...
                        <p className="font-bold text-brand-navy truncate">{paymentStatus}</p>
                     </div>
                     <div>
                        <p className="text-slate-400 font-bold uppercase text-[10px]">Akses</p>
                        <p className="font-bold text-brand-navy">{accessLabel}</p>
                     </div>
                     <div>
                        <p className="text-slate-400 font-bold uppercase text-[10px]">Berlaku</p>
                        <p className="font-bold text-brand-navy">{premiumUntil}</p>
                     </div>
                     <div>
                        <p className="text-slate-400 font-bold uppercase text-[10px]">Tindakan</p>
                        <p className="font-bold text-brand-navy truncate">{nextActionLabel}</p>
                     </div>
                  </div>
               </div>
               <button className="hidden sm:flex p-3 card-premium text-slate-600 relative items-center justify-center self-start" aria-label="Notifikasi akun">
                  <Bell size={20} />
